...
import { CarFront, CheckCircle2, Clock3, PlusCircle, XCircle } from "lucide-react";
...
export default function SellerDashboardPage() {
  ...
  const stats = useMemo(() => ({ total: cars.length, pending: cars.filter((car) => car.moderationStatus === "Pending").length, approved: cars.filter((car) => !car.moderationStatus || car.moderationStatus === "Approved").length, rejected: cars.filter((car) => car.moderationStatus === "Rejected").length }), [cars]);
  const cards = [{ label: ar ? "مركباتي" : "My vehicles", value: stats.total, icon: CarFront }, { label: ar ? "في انتظار الموافقة" : "Pending approval", value: stats.pending, icon: Clock3 }, { label: ar ? "مقبولة" : "Approved", value: stats.approved, icon: CheckCircle2 }, { label: ar ? "مرفوضة" : "Rejected", value: stats.rejected, icon: XCircle }];
