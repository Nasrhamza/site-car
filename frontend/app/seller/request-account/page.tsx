  ...
  return <div className="container-premium section-spacing">
    ...
    <div className="mx-auto grid max-w-6xl items-start gap-6 lg:grid-cols-2">
      <section className="rounded-[32px] bg-zinc-950 p-6 text-white shadow-2xl sm:p-9 lg:sticky lg:top-24">
        ...
        <form className="mt-7 grid gap-4" onSubmit={async (event) => { event.preventDefault(); setLoginLoading(true); setLoginError(""); try { const { data } = await api.post("/auth/seller/login", loginForm); setSession(data.accessToken, data.user); router.replace("/seller"); router.refresh(); } catch (error: any) { setLoginError(error?.response?.data?.message || (ar ? "تعذر تسجيل الدخول" : "Unable to sign in")); } finally { setLoginLoading(false); } }}>
