...
export default function SellerAddCarPage() {
  ...
  const handleFiles = (event: React.ChangeEvent<HTMLInputElement>) => { const incoming = Array.from(event.target.files || []); setFiles((current) => { const unique = new Map<string, File>(); [...current, ...incoming].forEach((file) => unique.set(`${file.name}-${file.size}-${file.lastModified}`, file)); return Array.from(unique.values()).slice(0, MAX_CAR_IMAGES); }); event.target.value = ""; };

  return <section className="mx-auto w-full max-w-6xl py-4"><div className="mb-5"><p className="text-sm font-black uppercase tracking-[.25em] text-brand">{ar ? "مخزون البائع" : "Seller inventory"}</p><h1 className="mt-2 text-4xl font-black">{ar ? "إضافة مركبة" : "Add a vehicle"}</h1><p className="mt-2 text-sm text-zinc-500">{ar ? "أكمل الإعلان ليصل إلى الإدارة للموافقة." : "Complete the listing. It will be sent to the administrator for approval."}</p></div>
    ...
    <form ref={formRef} className="rounded-[28px] border border-zinc-200 bg-white p-4 dark:border-white/10 dark:bg-zinc-900 sm:p-6" onSubmit={async (event) => { event.preventDefault(); setSaving(true); setError(""); try { const data = new FormData(); appendAdminCarFields(data, form); files.forEach((file) => data.append("images", file, file.name)); await api.post("/cars", data); router.push("/seller/cars"); router.refresh(); } catch (requestError: any) { setError(requestError?.response?.data?.message || (ar ? "تعذر إضافة المركبة" : "Unable to add vehicle")); } finally { setSaving(false); } }}>
      ...
      {isLast ? <section className="mt-5 rounded-3xl border border-zinc-200 bg-zinc-50 p-5 dark:border-white/10 dark:bg-white/[.03]"><h2 className="font-black">{ar ? "صور المركبة" : "Vehicle photos"}</h2><input type="file" accept="image/*" multiple onChange={handleFiles} className="mt-4 w-full rounded-2xl border bg-white px-4 py-3 dark:bg-transparent" /><p className="mt-2 text-xs text-zinc-500">{ar ? "حتى 12 صورة. الصورة الأولى هي الغلاف." : "Up to 12 photos. The first is the cover."}</p>{files.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2">{files.map((file, index) => <div key={`${file.name}-${file.lastModified}`} className="flex items-center justify-between rounded-xl bg-white px-3 py-2 text-sm"><span className="truncate">{index + 1}. {file.name}</span><button type="button" onClick={() => setFiles((current) => current.filter((_file, itemIndex) => itemIndex !== index))} className="text-red-500"><X className="h-4 w-4" /></button></div>)}</div> : null}</section> : null}
      {error ? <div className="mt-5 rounded-2xl bg-red-50 p-4 text-sm text-red-600">{error}</div> : null}
      <div className="sticky bottom-3 z-20 mt-5 flex flex-wrap justify-between gap-3 rounded-2xl border border-zinc-200 bg-white/95 p-3 shadow-xl backdrop-blur dark:border-white/10 dark:bg-zinc-900/95"><div className="flex gap-2">{activeStep > 0 ? <button type="button" onClick={() => showStep(activeStep - 1)} className="inline-flex items-center gap-2 rounded-xl border px-4 py-3 text-sm font-bold"><ArrowLeft className="h-4 w-4" />{ar ? "السابق" : "Back"}</button> : null}<button type="button" onClick={() => router.push("/seller/cars")} className="rounded-xl border px-4 py-3 text-sm font-bold">{ar ? "إلغاء" : "Cancel"}</button></div>{isLast ? <button disabled={saving} className="rounded-xl bg-brand px-6 py-3 text-sm font-black text-white">{saving ? (ar ? "جار الإرسال..." : "Submitting...") : (ar ? "إرسال للموافقة" : "Submit for approval")}</button> : <button type="button" onClick={next} className="inline-flex items-center gap-2 rounded-xl bg-brand px-6 py-3 text-sm font-black text-white">{ar ? "التالي" : "Next"}<ArrowRight className="h-4 w-4" /></button>}</div>
    </form>
  </section>;
}
