    ...
    <div className="rounded-[28px] border border-zinc-200/70 bg-white p-4 shadow-premium dark:border-white/10 dark:bg-zinc-900 sm:p-5">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="بحث مباشر: الفئة، الماركة أو الموديل..."
              className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 py-3 pl-4 pr-11 text-right outline-none transition focus:border-brand dark:border-white/10 dark:bg-white/5"
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-zinc-500 dark:text-zinc-400">
          <span>{total} مركبة مطابقة</span>
          {search && <span className="rounded-full bg-brand/10 px-3 py-1 text-brand">البحث: {search}</span>}
          {params.category && <span className="rounded-full bg-zinc-100 px-3 py-1 dark:bg-white/5">{getCategoryLabel(params.category)}</span>}
          {params.fuelType && <span className="rounded-full bg-zinc-100 px-3 py-1 dark:bg-white/5">{getFuelTypeLabel(params.fuelType)}</span>}
        </div>
          ...
          >
            <div className="grid gap-4 pt-2 md:grid-cols-2 xl:grid-cols-5 lg:pt-0">
              ...
              >
                ...
                <option value="-price">السعر تنازلي</option>
