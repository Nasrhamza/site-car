...
export type AdminCarFormValues = {
  ...
  price: string; priceType: string; status: string; badgesText: string; description: string;
};
...
export const emptyAdminCarForm: AdminCarFormValues = {
  ...
  year: "", mileage: "", fuelType: "", gearbox: "", transmission: "",
  exteriorColor: "", interiorColor: "", engineCapacity: "", regionalSpecs: "",
  cylinders: "", powerHp: "", steeringSide: "", doors: "", seats: "",
  wheelSize: "", location: "", exportStatus: "", serviceHistory: "", safetyText: "",
  price: "", priceType: "Prix fixe", status: PRODUCT_STATUS_OPTIONS[0], badgesText: "", description: ""
};
...
export const adminCarPayloadFields: Array<keyof AdminCarFormValues> = [
  ...
  "wheelSize", "location", "exportStatus", "serviceHistory", "price", "priceType", "status", "description"
];
...
export function appendAdminCarFields(formData: FormData, form: AdminCarFormValues) {
  ...
  const category = form.bodyType === "Truck" ? "Camions" : form.bodyType === "Bus" ? "Bus / Minibus" : DEFAULT_VEHICLE_CATEGORY;
...
export function AdminCarFields({ form, setForm, aedToTndRate, exchangeDate, section = "all" }: { form: AdminCarFormValues; setForm: Dispatch<SetStateAction<AdminCarFormValues>>; aedToTndRate: number; exchangeDate?: string | null; section?: AdminCarFieldsSection }) {
  ...
  const c = ar ? {
    ...
    name: "اسم الإعلان", brand: "الماركة", model: "الموديل", bodyType: "نوع الهيكل", trim: "التجهيز (اختياري)",
    ...
    exportStatus: "حالة التصدير", service: "سجل الصيانة", safety: "مزايا السلامة (مفصولة بفواصل)", price: "السعر بالدرهم الإماراتي", priceType: "طريقة عرض السعر", status: "حالة الإعلان", badges: "الشارات (مفصولة بفواصل)", description: "الوصف الكامل", choose: "اختر", optional: "اختياري"
  } : {
    listing: "Listing information", specs: "Specs & features", commercial: "Price & status", descriptionSection: "Description & content",
    name: "Listing title", brand: "Make", model: "Model", bodyType: "Body type", trim: "Trim (optional)",
    ...
    exportStatus: "Export status", service: "Service history", safety: "Safety features (comma separated)", price: "Price in AED", priceType: "Price display", status: "Listing status", badges: "Badges (comma separated)", description: "Full description", choose: "Select", optional: "Optional"
  };
  ...
  return <div className="grid gap-5">
    ...
    {section === "all" || section === "specs" ? <FieldGroup title={c.specs} columns={3}>
      <SelectField label={c.fuel} name="fuelType" value={form.fuelType} onChange={update} options={FUEL_TYPE_OPTIONS} emptyLabel={c.choose} renderLabel={(value) => ar ? getFuelTypeLabel(value) : optionLabel(value)} required />
      ...
      <SelectField label={c.engine} name="engineCapacity" value={form.engineCapacity} onChange={update} options={ENGINE_CAPACITY_OPTIONS.map(String)} emptyLabel={c.optional} renderLabel={(value) => `${Number(value).toFixed(1)} L`} />
      <SelectField label={c.regional} name="regionalSpecs" value={form.regionalSpecs} onChange={update} options={REGIONAL_SPECS_OPTIONS} emptyLabel={c.optional} />
      ...
      <SelectField label={c.steering} name="steeringSide" value={form.steeringSide} onChange={update} options={STEERING_SIDE_OPTIONS} emptyLabel={c.optional} renderLabel={optionLabel} />
      ...
      <SelectField label={c.exportStatus} name="exportStatus" value={form.exportStatus} onChange={update} options={EXPORT_STATUS_OPTIONS} emptyLabel={c.optional} renderLabel={optionLabel} />
